...
public class GameLogic : MonoBehaviour
{
	...
	public GameObject startUI, restartUI, instruct;
	...
	public void StartPuzzle()
	{
		...
		//light up all the spheres at once at the beginning of the display
		foreach (GameObject sphere in puzzleSpheres) {
				sphere.GetComponent<LightUp> ().PatternLightUp (2f);
			Debug.Log ("Lighting each");
		}

		// Call the DisplayPattern() function repeatedly.
		CancelInvoke("DisplayPattern");
		InvokeRepeating("DisplayPattern", 7, puzzleSpeed);
	...
	void DisplayPattern()
	{
		// reset solve index in case user was selecting stuff before the pattern was done
		currentSolveIndex = 0;
